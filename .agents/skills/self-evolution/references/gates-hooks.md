...
- **文档同步机制自洽（checklist ⇄ turn-backstop/reviewer ⇄ 反馈 ⇄ 资产类型）**（ADR-0012 后的新拓扑）：判据源 = `docs/harness/doc-sync-checklist.md`（数据文件、唯一真相源）；检测 = `turn-backstop.sh` 读该表 `🔴手` 行（Haiku (f) 项）+ `doc-sync-reviewer` 子 agent；送达 = 漂移写 `optimization-log`（`- [ ]` 状态）经 `correction-nudge` 下一轮反馈主 agent。新增一类资产时 checklist 要有对应行、turn-backstop (f) 例子要跟上，否则该类漂移没人检测。**预防层（原 doc-sync skill 主动 checklist）已随 ADR-0012 取消、只留检测+反馈兜底；Codex 无自动触发（靠 make verify + 手动派 reviewer）。**（并入自进化闭环、不另建 drift/ 区，见 ADR-0006。）
