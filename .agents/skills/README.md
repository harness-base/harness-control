...
| prd-elicitation | 产出需求（而非实现需求）：通过引导式多轮对话把想法或现有系统理清楚，分阶段产出用户故事 → PRD → 可选交互原型，每阶段用户确认。用户说"做需求/PRD/原型""把这个项目/功能先想清楚再动手""理一理需求""出个原型"时用。它是 dev（实现需求）的【上游】——产物独立存放、与实现体系松耦合（实现不强依赖它，有则可衔接）。 |
