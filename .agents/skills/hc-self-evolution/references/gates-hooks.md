...
- **文档同步机制自洽（checklist ⇄ turn-backstop/reviewer ⇄ 反馈 ⇄ 资产类型）**（ADR-0012 后的新拓扑）：判据源 = `docs/harness/doc-sync-checklist.md`（数据文件、唯一真相源）；检测 = `turn-backstop.sh` 读该表 `🔴手` 行（Haiku (f) 项）+ `hc-doc-sync-reviewer` 子 agent；送达 = 漂移写 `optimization-log`（`- [ ]` 状态）经 `correction-nudge` 下一轮反馈主 agent。新增一类资产时 checklist 要有对应行、turn-backstop (f) 例子要跟上，否则该类漂移没人检测（正例：`SANDBOX_CONTRACT.md` 落地时 checklist 加了对应行——契约改动要联动 hc-create-sandbox skill / `verification-audit` 字段清单）。**预防层（原 doc-sync skill 主动 checklist）已随 ADR-0012 取消、只留检测+反馈兜底；Codex 无自动触发（靠 make verify + 手动派 reviewer）。**（并入自进化闭环、不另建 drift/ 区，见 ADR-0006。）
...
cat  $ROOT/.claude/settings.json                  # PreToolUse(Bash→hook-policy) / Stop(stop-check) / UserPromptSubmit(correction-nudge)
...
bash $ROOT/scripts/stop-check.test.sh
...
- **是不是 load-bearing**：找它的 `*.test.sh` 或 `--check` 自测。没有自测，或自测不含「故意改坏→变红」的反例 → 判**花架子**。`hook-policy.test.sh` 有 `expect_block`/`expect_ok`；`rules-index.sh` 的 `check_eval_pointers` 变异自证（注入 `eval:999`→FAIL→还原 PASS）记录在 `docs/eval/task-reviews/20260626T014408Z-*/decision.md`；新硬闸（`test-cases-audit` / `designs-audit` / `verification-audit` / `stop-check` / `correction-nudge` / `lessons-promote-check`）各带 `.test.sh` 且进 verify——这些是合格样板。
...
- **codex 等价缺不缺**：逐条问「这个 hook 挂在哪层」。挂 `.claude/settings.json`（PreToolUse hook-policy、Stop stop-check、UserPromptSubmit correction-nudge）的 → **Codex 侧已接原生 `[hooks]`**（`.codex/config.toml`，ADR-0025，**验证 PENDING 待真机**——验证通过前 `.githooks` + CI 仍是硬兜底；判据从「有没有机制」改为「接线验证过没有」）。〔旧口径存档：接线前 Codex 侧无软 hook，只能靠 `.githooks/pre-commit`（也调 hook-policy）+ pre-push（跑 verify）+ CI 兜底〕。硬层判据仍适用：该红线在 `.githooks/`/CI 有没有等价拦截？没有 = 缺口。
- **该机器化的是否靠自觉**：能确定性判的检查若散在 `AGENTS.md` 当口头规则、没进脚本 → 判缺口（如「eval 标记必须指向存在考题」曾只是约定，后来才固化进 `rules-index.sh --check`）。
- **软当硬用**：红线只挂软层（settings.json Stop）而硬层无等价 → 判 severity 错配。`stop-check.sh` 自己诚实标了局限：`eval:` 标注与 `## Review` 收尾段都由 agent 在 todo.md 自报（只在补了 Review 段=收尾时才拦，防 mid-task 误拦），**故意标 eval: 不要或不补 Review 段仍能绕**——这类「半强制」必须在 HOOKS.md 写明（见 `docs/harness/HOOKS.md:47` 局限段），不能假装是 100% 硬门禁。
...
- **检查不 load-bearing，把垃圾当结果落库**（`tasks/lessons.md` 2026-06-26「兜底把超长 transcript 喂给 Haiku」）：turn-backstop 把 Haiku 返回的 `Prompt is too long` 当成「发现」追加进 `optimization-log.md`——检查在「跑」但产出是噪声。修法：输出**只认预期格式**（findings 必须 `^\[类别\]`），报错/NONE/空一律不记（见 `turn-backstop.sh:92` 附近的输出过滤）。同型续集：headless 预算曾设 0.03 偏紧，超预算报错被 `2>/dev/null` 吞掉→长期 0 产出没人发现；现 `BACKSTOP_BUDGET` 默认 0.20 + 诊断日志（`dlog`）记录 headless exit / 预算 / 0 产出发生点——**best-effort 兜底也要有自己的可观测性**，否则「跑了没跑」都不知道。
- **测试不绑真实链 = 空转测试**（`tasks/lessons.md` 2026-06-24「14 轮对抗评审」补记）：R9 的 metrics 测试不绑真实链、R10 的 DLQ 测试喂合成不可达输入——「跑过」但没测到真东西。修法：补测试一律 **mutation 自证**（改坏被测逻辑确认变红），并问「喂的输入生产路径真会产生吗」。
...
- **soft hook 的「半强制」局限**（`docs/harness/HOOKS.md:47`、`scripts/stop-check.sh:25`）：Stop hook 的强制 eval 靠 agent 在 todo.md 自报 `eval: 要` + 自补 Review 段，故意标不要/不补可绕——本仓已诚实标注，审查时确认这类局限**写明了**、没被吹成硬门禁即可。
...
- **写产物区硬闸的范式**：占位/含糊判定照 `scripts/verification-audit.sh`（**显式三态 fail-closed**：真命令 / `PENDING: 理由` / `N/A: 理由`，静默空·裸占位词一律红，不是"不在黑名单就放行"）；登记双向一致 + 内容硬闸照 `scripts/designs-audit.sh`（零 TBD 七词表）/ `scripts/test-cases-audit.sh`（covers 闭合 + 矩阵格 fail-closed）。
