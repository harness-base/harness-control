...
审查 harness 的「质量闸门」是否真闸得住：命中判据（多步改产物/写 ADR/动业务代码）/关键点收尾该评必评、规则有考题守、考题↔规则不悬空、评审不走过场。

## 规范（健康长什么样 / 不变量）

- **该评必评（rule-0005）**：命中判据（多步改产物 / 写了 ADR / 动业务代码）的任务与关键决策点收尾前必过 eval（todo 标 `eval: 要`，ADR-0025）；轻量问答 / 琐碎修补不触发。
...
- **缺口·该评没评**：标 `eval: 要` 的任务收尾 `task-reviews/` 无对应产出，或 `decision.md` 缺 verdict/证据/综合分档 → 走过场，补评。
