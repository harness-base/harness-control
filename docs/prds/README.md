...
由 `hc-prd` skill 分阶段产出的 **用户故事 → PRD →（可选）交互原型**，是 `hc-dev`（实现需求）的**上游**。与实现体系**松耦合**：实现（`hc-dev`）不强依赖此处——有 PRD 则吃、无则可独立干（ADR-0023）。流程见 `../decisions/0007-prd-workflow-redesign.md`。

- 模板：`templates/user-story.md`、`templates/prd.md`
- 账本：`index.yaml`（每个 PRD 一条：`id` / `dir`（目录名，`prds-audit` 按此键校验）/ `title` / `prd_status`）
