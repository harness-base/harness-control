...
由 `prd-elicitation` skill 分阶段产出的 **用户故事 → PRD →（可选）交互原型**，是 `feature-delivery`（实现需求）的**上游**。与实现体系**松耦合**：feature 实现不强依赖此处，有 PRD 时可派生 feature 包衔接。流程见 `../decisions/0007-prd-workflow-redesign.md`。
