...
| 命令 | 脚本 | 干什么 |
|---|---|---|
| `make verify` | `verify-control-plane.sh` | 控制面总自检：结构 + 文档自检 + hook policy 自测 + 落文档提醒自测 + eval 资产 + 各类索引漂移检查。**日常一键体检走这个。** |
...
| `designs-audit.test.sh` | `designs-audit.sh` |
| `verification-audit.test.sh` | `verification-audit.sh` |
