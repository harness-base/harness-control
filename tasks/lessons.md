...
## 2026-07-07：判断 skill"重不重 / 该不该拆"只量了 SKILL.md 一个文件，漏了散在子 agent+真相源里的重量
...
## 2026-07-07：讲 hc-test 下一步 uplift 时，把"写脚本"和"回归执行"糊成一坨——用户之前定过要分两步推 <!-- opt: seen -->
...
## 2026-07-07：讲 hc-test 状态时把"跑不跑"埋在"脚本线/回归线占位"里，用户得反问才对上 <!-- opt: seen -->
...
## 2026-07-07：日常汇报里用了圈内黑话"消歧"，用户没听过要专门问，反而拖累沟通 <!-- opt: seen -->
...
## 2026-07-01：用户要"捋范围"，我甩了一整张分类全景表（标准/主观/下游/边界 + 术语），又把人淹了 <!-- opt: seen -->
...
## 2026-07-01：给推荐时把决策 tie-breaker 默认成"更快能用/MVP 先跑通"，用户要的是"完善好" <!-- opt: seen -->
...
## 2026-07-03：把"写测试脚本"和"执行测试"糊成一个流程，用户纠正：产出与执行是两个流程 <!-- opt: seen -->
...
## 2026-07-03：把改 bug 的"复现"写成与"读代码"并列的可选路径，用户纠正：复现是根因的验证器、必须闭环 <!-- opt: seen -->
...
## 2026-07-03：把用户的"纠正某个点"当成"全案批准"，没等说完就全建了 <!-- opt: seen -->
...
## 2026-07-03：排"下一步"按"哪个前置刚通"而不是按开发流程顺序，被纠正（coding 在测试脚本前） <!-- opt: seen -->
...
## 2026-07-03：又推荐"实战走一遍"当下一步，用户第二次拉回"先把流程建完" <!-- opt: seen -->
...
## 2026-07-01：修 F-1 时往 shell-单引号包裹的 awk 程序注释里塞了 `'  '` 单引号，提前闭合、整个 awk 失灵 <!-- opt: seen -->
...
## 2026-07-01：占位→实现的接线漏了"总监总谱 skill"这个真正的功能入口（且 rule-0012 复刻表再次漂移） <!-- opt: seen -->
