...
## 2026-07-10：todo 项写着 PROJECT_ONBOARDING 打了 [x]，实际那文件一个字没动——打勾前没对 git status
...
## 2026-07-09：为回应"会不会太重"临场发明"脚本先 diff"，被戳穿格式根本不同——别为安抚担心加不成立的机制
...
## 2026-07-09：设计"取数"环节默认让 agent 临场提取，用户纠正为脚本化——取数用脚本、判断用 agent
...
## 2026-07-09：对抗修复轮改了新文件，ADR 受影响栏/连带清单不回填——同型第二次被 eval 逮
...
## 2026-07-09：提"L0-L6 收敛三档"还是档位思维，没看出"读多少"半边已被渐进式引用结构整体取代
...
## 2026-07-08：把"历史文档不改写"当处理完毕，漏了"旧文档读者怎么知道口径变了"——不改写 ≠ 不导航
...
## 2026-07-08：给 features 退役提方案时只给了"换锚保硬门禁"两选项，漏了用户的一贯哲学"松耦合、提示不强耦"
...
## 2026-07-08：判"features 体系没废弃"只看引用还挂着，没看实质工作流早已绕开它——形式活着 ≠ 实质活着
