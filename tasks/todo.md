...
- **血缘（改动面）**：7 skill（hc-dev/hc-prd/hc-tech-design/hc-onboard/hc-test/hc-create-sandbox/hc-add-rule）review 步 + 8 reviewer 双栈（16 文件；hc-doc-sync-reviewer 钩子驱动不在内）+ CURRENT_STATUS + doc-sync-checklist + ADR（config.toml 后判无需改——reviewer description 未动）。
