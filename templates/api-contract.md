...
> 每个接口一块，顺序与端点索引一致。块内固定：**描述 + 鉴权 → 请求 → 成功响应（含 Mock 样例）→ 错误响应 →（写端点补）幂等 / 并发 → 关联**（读端点 `GET` 无幂等 / 并发段）。
...
**幂等 / 并发**（写端点必填——`GET` 读端点略；`hc-tech-design-reviewer` ③ 硬核这一项，无槽易首轮误判）

- 幂等：<幂等键 / 唯一约束 / 去重策略；重复提交 / 客户端重试会不会造重复写，如"以 `name` 唯一约束去重，冲突返回 409；或带幂等键 `Idempotency-Key`，重复请求返回首次结果">
- 并发写：<乐观锁 / 行锁 / 冲突返回，如"`UPDATE … WHERE version = ?`，命中 0 行即冲突、返回 409 `CONFLICT`">
